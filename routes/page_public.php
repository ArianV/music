<?php
// ====== page_public.php (drop-in) ======
// Public view of a single page by slug or id.
// Route examples (both work):
//   /pages/123
//   /pages/lil-uzi-vert-20-min
require_once __DIR__ . '/config.php';

// -------- Resolve key from URL --------
$path = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH) ?? '';
$key  = null;

// Expected route: /pages/{key} or /pages/{key}/
if (preg_match('#/pages/([^/]+)/?$#', $path, $m)) {
  $key = rawurldecode($m[1]);
} elseif (isset($_GET['id'])) {
  $key = $_GET['id']; // fallback query param
}
...
// Enforce "published" for non-owners
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    header {
      ...
      padding:12px 16px; background:#111318; border-bottom:1px solid #1f2430;
    ...
    .btn-pill {
      ...
      font-weight:700; font-size:14px; line-height:1; border:1px solid transparent;
    ...
    footer {
      padding:14px 16px; color:#9ca3af; border-top:1px solid #1f2430; background:#111318;
    }
  </style>
</head>
<body>
...
<main>
  <article class="card">
    ...
    <?php if (!empty($links)): ?>
      ...
      ?>
        <a class="btn-pill <?= $isSpotify ? 'btn-spotify' : 'btn-default' ?>"
